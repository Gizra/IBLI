...
if length(ind) > 0 % we have an intercept
    if ind ~= 1
    warning('sempanel_gmm: intercept must be in 1st column of the x-matrix');
    end;
    iflag = 1;
end;
...
JT = ones(T,T);
Qon = kron(eye(T) - JT/T),speye(n));
Q1n = kron(JT/T,speye(n));
