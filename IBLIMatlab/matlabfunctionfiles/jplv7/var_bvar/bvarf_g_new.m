...
%%%% ADAM's NOTE: I wanted to include a trend but this didn't work in
%%%% bvarf_g because either: 1) the dimension of the x vector was too large
%%%% for the estimation routine; or 2) the x vector didn't contain the
%%%% necessary elements for the forecast period.
if nx > 0
    xxxx=x;
    for delete = 1:nfor
        [m2,n2]=size(xxxx);
...
result = bvar_g(yvec,nlag,ndraw,nomit,prior,xxxx);
else
