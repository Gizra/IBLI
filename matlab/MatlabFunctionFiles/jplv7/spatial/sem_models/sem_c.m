...
%          rho = beta(a1,a2)
%-------------------------------------------------------------
% USAGE: results = sem_c(y,x,W,prior)
% where: y = dependent variable vector (nobs x 1)
%        x = independent variables matrix, with no intercept term (nobs x nvar)
...
ind = find(tst == n);
if length(ind) > 0
    error('sem_c: you must NOT have an intercept term in the x-matrix');
end;
...
elseif n3 ~= n4
error('sem_c: W matrix is not square');
elseif n3~= n
error('sem_c: W matrix is not the same size at y,x');
end;
...
[nu,d0,rho,sige,rmin,rmax,detval,ldetflag,order,iter,a1,a2,g] = sem_parse(prior,x);
...
epe = g1*epe;
Qvec = interp1(rgrid',qvect,detval(:,1),'spline');
Qvec = g2*Qvec;
detx = iota*(k/2)*lg2;
...
no2 = nobs/2;
nm1o2 = (nobs-1)/2;
den = detx + log(bprior) + detval(:,2) - nm1o2*log(epe + Qvec);
...
function [nu,d0,rho,sige,rmin,rmax,detval,ldetflag,order,iter,a1,a2,g] = sem_parse(prior,x)
% PURPOSE: parses input arguments for sem_c models
% ---------------------------------------------------
...
elseif ldetflag == -1 % the user fed down a detval matrix
    time1 = 0;
        ...
        if detval == 0
            error('sem_c: wrong lndet input argument');
        end;
        [n1,n2] = size(detval);
        if n2 ~= 2
...
% den = logdetx*iota +  log(bprior) + detval(:,2) - no2*log(z) - no2*log(Qvec);;
kterm = gammaln(nm1o2) -no2*log(pi);
gterm = 1/(1+g);
