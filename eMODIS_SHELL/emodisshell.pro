...
function eModisShell , settingsFilePath
...
;http://earlywarning.usgs.gov//ftp2/eMODIS/east/east1402.zip
  spawn, 'unzip '+[rawDataPath+'/east'+ySTR+mSTR+'.zip']+' -d '+rawDataPath , /HIDE
  spawn, 'rm '+rawDataPath+'/*.zip', /HIDE
  FOR S = 6*M-4,6*M,2 DO BEGIN
        sSTR = STRMID('0'+STRCOMPRESS(STRING(S), /REMOVE_ALL), 1,2, /REVERSE_OFFSET)
       ...
       CREATE_HEADER , rawDataPath+'/'+'20'+ySTR+sSTR+'.img',SUBSET(2),SUBSET(3),1,'{NDVI}'
       ENDIF
  ENDFOR
  
  spawn, 'rm '+rawDataPath+'/*.tif', /HIDE
  spawn, 'rm '+rawDataPath+'/*.tfw', /HIDE
...
FOR k=0,-1  DO BEGIN ; ; ; - - - - REMEMBER TO TAKE BACK TO 9 - - - - ; ; ; 
 ...
 mSTR = STRCOMPRESS(string(toUpdateMonth),/REMOVE_ALL)
  ...
  IF FILE_TEST(rawDataPath+'/ea'+mSTR+ySTR+'m.tif') THEN BEGIN
       ...
       CREATE_HEADER ,rawDataPath+'/'+'20'+ySTR+STRMID('0'+mSTR, 1,2, /REVERSE_OFFSET)+'.img',SUBSET(2),SUBSET(3),1,'{NDVI}'
   ENDIF
