...
3. after repository is completly up-to-date : stack all images together and construct one large multi-temporal image names. the stacked image will be at : WorkingFolder/PROCESSED/eMODIS_FEWS_Kenya.bil
4. after images are stacked we calculate the seasonal mean and std as well as do a quick diagnostics to through out bad pixels  (mainly low data variance). all those files are save at : WorkingFolder/PROCESSED/eMODIS_FEWS_Kenya_avg.bil , WorkingFolder/PROCESSED/eMODIS_FEWS_sd.bil ,WorkingFolder/PROCESSED/eMODIS_FEWS_dia.bil. after calculating all those we can calculate the normalized zNDVI per pixel. the file is saved at : WorkingFolder/PROCESSED/eMODIS_FEWS_Kenya_Z.bil
