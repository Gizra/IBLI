...
;
; INPUTS:
;  - monthly zipped files that contain 6 TIFF-files for the 6 partly overlapping dekads in a month
;
; USAGE:
;   eMODIS_ORGANIZE_OP, startYear, endYear, startMonth, endMonth, dataPath, workPath
; 
; EXAMPLE:
;   1. Organize from June 2013 to January 2014
;      eMODIS_ORGANIZE_OP, 2014, 2014, 1, 1, 'F:\DATA\eMODIS_FEWSNET\originalData\', 'F:\DATA\eMODIS_FEWSNET\data\'
;     
; OUTPUT:
...
;       - put start/end month/year as arguments of the procedure
...
PRO eMODIS_ORGANIZE_OP, startYear, endYear, startMonth, endMonth, dataPath, workPath
...
FOR year=startYear, endYear, 1L DO BEGIN
  ...
  yearWorkPath = workPath+sYear+dirSep
  IF FILE_TEST(yearWorkPath) eq 0 THEN FILE_MKDIR, yearWorkPath
  ; access the correct months
  IF (year eq startYear) THEN startMonth2=startMonth ELSE startMonth2=1
  IF (year eq endYear) THEN endMonth2=endMonth ELSE endMonth2=12

  FOR month=startMonth2, endMonth2, 1L DO BEGIN
    sMonth = STRMID('0'+STRCOMPRESS(MONTH, /REMOVE_ALL), 1,2, /REVERSE_OFFSET)
    zipFileName = dataPath+sYear+dirSep+region[0]+sYearShort+sMonth+'.zip'
    IF FILE_TEST(zipFileName) eq 1 THEN BEGIN
      ;spawn, 'unzip '+zipFileName +' -d '+dataPath, /HIDE
      FILE_UNZIP, zipFileName, dataPath
      ...
      fileList = FILE_SEARCH(dataPath+'*.t*')
...
END ; {PROCEDURE eMODIS_ORGANIZE_OP}
