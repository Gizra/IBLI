...
% % [samples values of rho to produce a posterior distribution]
...
% a grid of rho values from options.rmin to options.rmax
% a single value for # neighbors, option.neigh is used in this function
%-------------------------------------------------------------
% USAGE: results = mess_g2(y,x,options,ndraw,nomit,prior,start)
...
%  options.neigh = # of neighbors to use in constructing D
%  options.xflag = 0 for S*y = X*b + e,         model (default)
%                = 1 for S*y = [i X D*X]*b + e, model
%  options.rmin  = minimum value of rho to use in discounting 
%                  (0 < rho < 1), (default 0.5)
...
%          results.amax   = amax: max like alpha + 3*std(alpha) (or user input)
%          results.amin   = amin: max like alpha - 3*std(alpha) (or user input)      
...
% --------------------------------------------------------------
% SEE ALSO:  mess_g2d, mess_g, mess_g3, messv_g, prt, c_mess, mess
% --------------------------------------------------------------
...
if n1 ~= n
error('mess_g2: x-matrix contains wrong # of observations');
end;
...
if nargin == 7
    if ~isstruct(start)
        error('mess_g2: must supply starting values in a structure');
    end;
 ...
 for i=1:nf
    if strcmp(fields{i},'b')
       ...
       if n1 ~= k
        error('mess_g2: starting beta values are wrong');
       elseif n2 ~= 1
        error('mess_g2: starting beta values are wrong');
       end;
    elseif strcmp(fields{i},'sig')
       ...
       if n1 ~= 1
        error('mess_g2: starting sige value is wrong');
       elseif n2 ~= 1
        error('mess_g2: starting sige value is wrong');
       end;
    elseif strcmp(fields{i},'a')
       ...
       if n1 ~= 1
        error('mess_g2: starting alpha value is wrong');
       elseif n2 ~= 1
        error('mess_g2: starting alpha value is wrong');
       end;
    end;
...
% parse options structure
    if ~isstruct(options)
        error('mess_g2: must supply option values in a structure');
    end;
...
% parse prior structure variable inputs        
    if ~isstruct(prior)
    error('mess_g2: must supply the prior as a structure variable');
    end;
...
error('Wrong # of arguments to mess_g2');
end;
...
[checkk,junk] = size(c);
if checkk ~= k
error('mess_g2: prior means are wrong');
elseif junk ~= 1
error('mess_g2: prior means are wrong');
end;
...
[checkk junk] = size(T);
if checkk ~= k
error('mess_g2: prior bcov is wrong');
elseif junk ~= k
error('mess_g2: prior bcov is wrong');
end;
...
if checkk ~= 1
error('mess_g2: prior alpha is wrong');
elseif junk ~= 1
error('mess_g2: prior alpha is wrong');
end;
...
[checkk junk] = size(S);
if checkk ~= 1
error('mess_g2: prior acov is wrong');
elseif junk ~= 1
...
if llflag ~= 2;
error('mess_g2: no lattitude-longitude coordinates input');
end;
...
% find maxl alpha using ? rho value ( 1 used here, the default);
...
iter = 1;
          ...
          xmat = squeeze(Sxmat(:,:,indexr));
          % get Sy for free
          Ycap = squeeze(Symat(:,:,indexr));
          ...
          % metropolis step to get alpha update
          if pflag == 0
          alphax = c_mess2(alpha,y,xmat,Symat,bhat,sige,rho,rmat); 
          elseif pflag == 1
          alphax = c_mess2(alpha,y,xmat,Symat,bhat,sige,rho,rmat,palpha,S);
          end;
          accept = 0;
          alpha2 = alpha + cc*randn(1,1);
          while accept == 0
           ...
           if pflag == 0
           alphay = c_mess2(alpha2,y,xmat,Symat,bhat,sige,rho,rmat);
           elseif pflag == 1
            alphay = c_mess2(alpha2,y,xmat,Symat,bhat,sige,rho,rmat,palpha,S);
           end;          
          ru = unif_rnd(1,0,1);
          if ((alphay - alphax) > exp(1)),
          p = 1;
...
% update rho using metroplis-hastings step
          rhox = r_mess2(rho,y,xmat,Symat,bhat,alpha,rmat); 
          rho2 = unif_rnd(1,rmin,rmax);

          rhoy = r_mess2(rho2,y,xmat,Symat,bhat,alpha,rmat);
           ru = unif_rnd(1,0,1);
          if ((rhoy - rhox) > exp(1)),
          p = 1;
          else,          
...
%  NOTE: called only by mess_g2
%  --------------------------------------------------
%  SEE ALSO: mess_g2, mess_g2d
% ---------------------------------------------------
...
error('c_mess2: Wrong # of inputs arguments');
end;

   

function cout = r_mess2(rho,ys,xs,Symat,beta,alpha,rmat);
% PURPOSE: evaluate the conditional distribution of rho 
%          for the Bayesian mess_g2 model
% ---------------------------------------------------
%  USAGE: cout = m_mess1(neigh,y,x,Symat,beta,alpha,rmat)
%  where:  rho = neighbors
%            y    = dependent variable vector
%            x    = explanatory variables matrix
%          Symat  = matrix from mess_g2 
%                   containing a mesh of Sy for rho values
%            beta = kx1 current bhat vector
