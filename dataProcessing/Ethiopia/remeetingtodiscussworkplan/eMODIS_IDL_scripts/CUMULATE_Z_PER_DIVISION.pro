...
IF dataSet eq 'eMODIS' THEN BEGIN
  ...
  numberPeriodsLong = 21       ; TO CREATE UPDATE LONG SEASON, FROM MARCH-MAY (9) OR MARCH-JUNE (12)
  ...
  numberPeriodsShort = 11            ; at time of update Nov2013 we only have 5 dekads for numberPeriodsShort
ENDIF

; access the input csv-file
inFile=dataPath+dataSet+'\z-scoring-first\update20140123\'+'zNDVI_aggregated_'+dataSet+'.csv'
; read data
...
outFile = dataPath+dataSet+'\z-scoring-first\update20140123\'+'zCumNDVI_aggregated_'+dataSet+'.csv'
outFile = dataPath+dataSet+'\z-scoring-first\update20140123\'+'zCumNDVI_aggregated_'+dataSet+'_S=11dekads.csv'
