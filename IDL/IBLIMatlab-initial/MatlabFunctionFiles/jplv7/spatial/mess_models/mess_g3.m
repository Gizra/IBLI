...
if n1 ~= n
error('mess_g3: x-matrix contains wrong # of observations');
end;
...
if nargin == 7
    if ~isstruct(start)
        error('mess_g3: must supply starting values in a structure');
    end;
 ...
 for i=1:nf
    if strcmp(fields{i},'b')
       ...
       if n1 ~= k
        error('mess_g3: starting beta values are wrong');
       elseif n2 ~= 1
        error('mess_g3: starting beta values are wrong');
       end;
    elseif strcmp(fields{i},'sig')
       ...
       if n1 ~= 1
        error('mess_g3: starting sige value is wrong');
       elseif n2 ~= 1
        error('mess_g3: starting sige value is wrong');
       end;
    elseif strcmp(fields{i},'a')
       ...
       if n1 ~= 1
        error('mess_g3: starting alpha value is wrong');
       elseif n2 ~= 1
        error('mess_g3: starting alpha value is wrong');
       end;
    end;
...
% parse options structure
    if ~isstruct(options)
        error('mess_g3: must supply option values in a structure');
    end;
...
% parse prior structure variable inputs        
    if ~isstruct(prior)
    error('mess_g3: must supply the prior as a structure variable');
    end;
...
error('Wrong # of arguments to mess_g3');
end;
...
[checkk,junk] = size(c);
if checkk ~= k
error('mess_g3: prior means are wrong');
elseif junk ~= 1
error('mess_g3: prior means are wrong');
end;
...
[checkk junk] = size(T);
if checkk ~= k
error('mess_g3: prior bcov is wrong');
elseif junk ~= k
error('mess_g3: prior bcov is wrong');
end;
...
if checkk ~= 1
error('mess_g3: prior alpha is wrong');
elseif junk ~= 1
error('mess_g3: prior alpha is wrong');
end;
...
[checkk junk] = size(S);
if checkk ~= 1
error('mess_g3: prior acov is wrong');
elseif junk ~= 1
error('mess_g3: prior acov is wrong');
end;
...
if llflag ~= 2;
error('mess_g3: no lattitude-longitude coordinates input');
end;
