...
% revised to use moments matrix approach of Kelley Pace, 9/2007
...
if sum(x(:,1)) ~= n
    ...
    ind = find(tst == n); % we do have an intercept term
        if length(ind) > 0
            error('sac_g: intercept term must be in first column of the x-matrix');
...
epe2 = epe*ones(nrho,1) - rgrid*(e'*W'*e) - rgrid*(e'*W*e) + rgrid.*rgrid*(e'*W'*W*e);


like =   -detval(:,2) + (n/2)*log(epe2);
